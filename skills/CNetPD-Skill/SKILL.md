...
`query.py` 每次调用都会先做远端版本检查。发现本地版本小于远端版本时，默认自动执行 `npx skills update CNetPD-Skill`，更新成功后重新执行原命令；更新失败则停止本次查询，避免继续用旧 skill 给出 API 结论。这个过程由查询脚本自己完成，不要求 Agent 在同一会话内重新加载 skill。
...
1. 若自动更新输出显示已重新执行原命令，以重新执行后的结果为准。
