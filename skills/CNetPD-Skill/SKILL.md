...
`query.py` 每次调用都会先做远端版本检查。发现新版本时，默认自动执行 `npx skills update CNetPD-Skill`，更新成功后重新执行原命令；更新失败则停止本次查询，避免继续用旧 skill 给出 API 结论。自动更新成功后，当前命令会用新脚本重跑，并输出 `AGENT_RELOAD_SKILL: <path>`；Agent 在继续回答前必须读取该路径的新版 `SKILL.md` 并按新版说明执行。新会话仍是最可靠的完整重载方式。
